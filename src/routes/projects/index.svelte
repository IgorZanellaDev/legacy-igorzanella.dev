...
<div class="p-2 sm:max-w-5xl" in:fade={{duration: 300}} >
	...
	<div class="w-full flex flex-col items-center justify-center perspective-500">
		{#each projects as project, i}
			<button on:click={() => {project.clicked = true; animateOut = true; setTimeout(()=> goto('/projects/'+project.slug), 400)}} class="bg-light w-5/6 m-2 rounded-lg flex flex-col focus:outline-none sm:m-4 {i % 2 ? "sm:flex-row-reverse" : "sm:flex-row"} {animateOut ? project.clicked ? "fade-out-fwd z-50" : "fade-out" : "hover:vibrate-1"} sm:w-full">
